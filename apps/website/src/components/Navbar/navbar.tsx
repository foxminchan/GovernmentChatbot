      ...
      <ul className="relative flex w-auto h-full left-44">
        ...
        {navItems.map((item) => (
          ...
          onMouseLeave={() => hideSubMenu(item.name)}
            ...
            {item.subMenu && subMenuVisibility[item.name] && <ul>
              {navSupport.map((item) => (
                <li key={item.name} className="w-56 h-12 text-white bg-japonica-400 hover:bg-japonica-500 hover:text-white">
                  <Link to="/" className="flex items-center w-full h-full px-3 text-left" >
                    <span className="text-xl font-medium">{item.name}</span>
                  </Link>
                </li>
              ))}
